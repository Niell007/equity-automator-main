...
CREATE INDEX IF NOT EXISTS idx_documents_user ON app_public.documents(user_id);
...
-- Documents policies
CREATE POLICY documents_select ON app_public.documents
    FOR SELECT TO authenticated
    USING (user_id = auth.uid());

CREATE POLICY documents_insert ON app_public.documents
    FOR INSERT TO authenticated
    WITH CHECK (user_id = auth.uid());

CREATE POLICY documents_delete ON app_public.documents
...
CREATE TRIGGER update_documents_updated_at
    BEFORE UPDATE ON app_public.documents
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at();
